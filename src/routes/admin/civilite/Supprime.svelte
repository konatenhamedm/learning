	...
	export let data: Record<string, string> = {};
	 ...
	 async function SaveFunction() {
        ...
        try {
            ...
            const res = await fetch(BASE_URL_LOCAL +`/delete/`+data?.id, {
                method: 'DELETE',
               /*  headers: {
	...
	<div class="flex items-center justify-center">
		<!-- <div class="flex justify-end"> -->
            ...
            {:else}
				...
				<Button type="submit" color="red" on:click={SaveFunction} class="mr-2">Supprimer</Button>
            {/if}
       <!--  </div> -->
		<Button color="alternative" on:click={() => (open = false)}>Annuler</Button>
	</div>
</Modal>